...
export default function RunGroupsButton({ labels }: { labels: { run: string; running: string } }) {
  ...
  async function run() {
    ...
    setSummary(
      `${body.proposed ?? 0} · ${body.edgesFound ?? 0} edges · ${body.hubPersonsIgnored ?? 0}+${body.hubAddressesIgnored ?? 0} hubs ignored`,
    );
    router.refresh();
  }
