...
export const DATASETS: DatasetSpec[] = [
  ...
  {
    datasetId: 'incoming_payment',
    entity: 'incoming_payment',
    labelTh: 'รายการรับชำระ',
    labelEn: 'Incoming payments',
    naturalKey: ['legalEntityCode', 'receiptRef'],
    columns: [
      { canonicalField: 'legalEntityCode', type: 'string', required: true, aliases: ['legal_entity', 'entity', 'company_code', 'bu', 'นิติบุคคล'], description: 'Legal entity that received the money' },
      { canonicalField: 'receiptRef', type: 'string', required: true, aliases: ['receipt_ref', 'receipt_no', 'payment_ref', 'document_no', 'เลขที่ใบเสร็จ', 'เลขที่รายการรับชำระ'], description: 'Receipt or payment reference in the source system' },
      { canonicalField: 'partySourceCode', type: 'string', required: false, aliases: ['customer_code', 'customer_no', 'source_code', 'party_code', 'รหัสลูกค้า'], description: 'Customer code — leave blank for a receipt nobody has attributed yet' },
      { canonicalField: 'payerName', type: 'string', required: false, aliases: ['payer_name', 'payer', 'remitter', 'ชื่อผู้โอน', 'ชื่อผู้ชำระ'], description: 'The name on the bank line, which is often not the legal name' },
      { canonicalField: 'paymentDate', type: 'date', required: true, aliases: ['payment_date', 'receipt_date', 'date', 'วันที่รับชำระ'], description: 'Date the money arrived' },
      { canonicalField: 'valueDate', type: 'date', required: false, aliases: ['value_date', 'clearing_date', 'วันที่มีผล'], description: 'Value / clearing date' },
      { canonicalField: 'amount', type: 'number', required: true, aliases: ['amount', 'paid_amount', 'value', 'จำนวนเงิน', 'ยอดรับชำระ'], description: 'Amount received' },
      { canonicalField: 'currency', type: 'string', required: false, aliases: ['currency', 'curr', 'สกุลเงิน'], description: 'Currency' },
      { canonicalField: 'channel', type: 'enum', required: false, enumValues: ['cheque', 'bill_of_exchange', 'bank_transfer', 'bill_payment', 'barcode', 'e_payment', 'direct_debit', 'cash', 'other'], aliases: ['channel', 'payment_method', 'method', 'ช่องทาง', 'วิธีชำระ'], description: 'How it was paid' },
      { canonicalField: 'sourceDocumentNo', type: 'string', required: false, aliases: ['invoice_no', 'applied_to', 'document_no_ref', 'reference_invoice', 'เลขที่ใบแจ้งหนี้', 'ใบแจ้งหนี้'], description: 'Invoice the source already applied it to — its presence is what raises the payment grain' },
      { canonicalField: 'reference', type: 'string', required: false, aliases: ['reference', 'bank_ref', 'remark', 'อ้างอิง', 'หมายเหตุ'], description: 'Bank reference or free text' },
      { canonicalField: 'chequeNo', type: 'string', required: false, aliases: ['cheque_no', 'check_no', 'เลขที่เช็ค'], description: 'Cheque number' },
      { canonicalField: 'chequeDueDate', type: 'date', required: false, aliases: ['cheque_due_date', 'check_due_date', 'วันที่เช็คถึงกำหนด'], description: 'Cheque due date' },
    ],
  },
  {
    datasetId: 'payment_exception',
    entity: 'payment_exception',
    labelTh: 'รายการรับชำระผิดปกติ / เช็คคืน',
    labelEn: 'Payment exceptions & returned cheques',
    naturalKey: ['type', 'reference', 'occurredAt', 'legalEntityCode'],
    columns: [
      { canonicalField: 'legalEntityCode', type: 'string', required: true, aliases: ['legal_entity', 'entity', 'company_code', 'bu', 'นิติบุคคล'], description: 'Legal entity the exception belongs to' },
      { canonicalField: 'partySourceCode', type: 'string', required: false, aliases: ['customer_code', 'customer_no', 'source_code', 'รหัสลูกค้า'], description: 'Customer code, when known' },
      { canonicalField: 'type', type: 'enum', required: true, enumValues: ['returned_cheque', 'reversal', 'mismatch', 'missing', 'failed_transfer', 'overpayment', 'unidentified_receipt'], aliases: ['type', 'exception_type', 'ประเภท', 'ประเภทรายการ'], description: 'What went wrong' },
      { canonicalField: 'amount', type: 'number', required: true, aliases: ['amount', 'value', 'จำนวนเงิน'], description: 'Amount involved' },
      { canonicalField: 'currency', type: 'string', required: false, aliases: ['currency', 'curr', 'สกุลเงิน'], description: 'Currency' },
      { canonicalField: 'occurredAt', type: 'date', required: true, aliases: ['occurred_at', 'date', 'return_date', 'event_date', 'วันที่เกิดเหตุ', 'วันที่เช็คคืน'], description: 'When it happened' },
      { canonicalField: 'reference', type: 'string', required: true, aliases: ['reference', 'cheque_no', 'bank_ref', 'เลขที่เช็ค', 'อ้างอิง'], description: 'Cheque number or bank reference — the handle everyone uses to talk about it' },
      { canonicalField: 'reasonCode', type: 'string', required: false, aliases: ['reason_code', 'return_code', 'รหัสเหตุผล'], description: "The bank's return reason code" },
      { canonicalField: 'reasonText', type: 'string', required: false, aliases: ['reason', 'reason_text', 'remark', 'เหตุผล', 'หมายเหตุ'], description: 'Reason in words' },
      { canonicalField: 'source', type: 'enum', required: false, enumValues: ['bank', 'erp', 'manual_entry'], aliases: ['source', 'แหล่งข้อมูล'], description: 'Where this was learned — manual_entry is normal, most returned cheques are known from a phone call first' },
      { canonicalField: 'status', type: 'enum', required: false, enumValues: ['open', 'resolved', 'written_off'], aliases: ['status', 'สถานะ'], description: 'Status (defaults to open)' },
    ],
  },
];
